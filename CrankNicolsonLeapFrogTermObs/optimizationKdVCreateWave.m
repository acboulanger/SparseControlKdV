...
%% Uncomment if goal is: create a specific wave at final time
    ...
    % observation domain
    observationdomain = floor(args.N*2.0/3.0):(args.N+1);
    args.Obs = zeros(args.N+1);
    for i=1:size(observationdomain,2)
        args.Obs(observationdomain(i), observationdomain(i)) = 1;
    end
    
    % control domain
    controldomain = floor(args.N/3):floor(args.N/2);
    noncontroldomain = ones(1,args.N+1);
    noncontroldomain(controldomain) = 0;
    %
    args.B = zeros(args.N+1);
    for i=1:size(controldomain,2)
        args.B(controldomain(i), controldomain(i)) = 1;
    end
    args.BT = (args.B)';
...
function y = solveState(q, args)
    ...
    y.spatial(1,:) = args.y0;
    y.spec(1,:) = yspec0;
    q = (B*(q'))';%effect of indicator function
...
function dy = solveTangent(q, y, dq, args)
    ...
    dq = (B*(dq'))';%effect of indicator function
